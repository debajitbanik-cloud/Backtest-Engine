...
class SuperTrendBBStrategy(Strategy):
    ...
    atr_tp_multiplier = 2.5  # Take profit multiplier
    ...
    def next(self):
        ...
        if not self.position:
            ...
            if (uptrend and
                ...
                # Use ATR multiplier for take-profit, but ensure it's at least middle BB
                tp = max(current_close + (current_atr * self.atr_tp_multiplier), current_bb_middle)
            ...
            # Short: Downtrend + SuperTrend bearish + price near/above upper BB
                ...
                sl = current_close + (current_atr * self.atr_sl_multiplier)
                # Use ATR multiplier for take-profit, but ensure it's at most middle BB
                tp = min(current_close - (current_atr * self.atr_tp_multiplier), current_bb_middle)
