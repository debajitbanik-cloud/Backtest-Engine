...
import { AgentConfig, AgentDecision, DailyReport, MarketData, Position, RiskMetrics, TradeSummary, OptimizationSuggestion, AgentPerformance } from '../types';
...
export class TradeMasterAgent extends BaseAgent {
  private tradeHistory: PositionWithMetadata[] = [];
  ...
  private optimizationQueue: OptimizationSuggestion[] = [];
  ...
  async analyze(
    ...
    if (today !== this.lastReportDate && this.shouldGenerateReport()) {
      ...
      return this.createDecision(
        'adjust',
        0.8,
        `Daily report generated: ${report.totalTrades} trades, ${(report.winRate * 100).toFixed(1)}% win rate, $${report.totalPnl.toFixed(2)} PnL`,
        { report, optimizations: this.optimizationQueue }
      );
    }
    ...
    return this.createDecision(
      'hold',
      0.6,
      `TradeMaster monitoring: ${this.tradeHistory.length} trades in history, ${optimizations.length} pending optimizations`,
      { optimizations, insights: this.insights.slice(-5) }
    );
  }
  ...
  recordAgentDecision(agentId: string, decision: AgentDecision, outcome: 'correct' | 'incorrect' | 'unknown'): void {
    ...
    perf.decisions++;
    if (outcome === 'correct') perf.correctDecisions++;
    perf.accuracy = perf.correctDecisions / perf.decisions;
  }

  private updateTradeHistory(positions: Position[]): void {
    for (const pos of positions) {
      if (pos.status === 'closed' && !this.tradeHistory.find(t => t.id === pos.id)) {
        this.recordTrade(pos as PositionWithMetadata);
      }
    }
  }

  private updateAgentPerformances(): void {
    const recentTrades = this.tradeHistory.slice(-100);
    const agentContributions: Map<string, number> = new Map();
  ...
  private generateInsights(position: Position): void {
  ...
  private generateDailyReport(riskMetrics: RiskMetrics): DailyReport {
    const today = new Date().toDateString();
  ...
  getPerformanceSummary(): { totalTrades: number; winRate: number; totalPnl: number; sharpeRatio: number; bestAgent: string; worstAgent: string } {
    ...
    for (const [id, perf] of this.agentPerformances) {
      if (perf.decisions > 10) {
        if (perf.accuracy > bestAcc) { bestAcc = perf.accuracy; bestAgent = id; }
        if (perf.accuracy < worstAcc) { worstAcc = perf.accuracy; worstAgent = id; }
      }
    }
