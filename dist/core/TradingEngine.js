...
class TradingEngine {
    ...
    isRunning = false;
    updateInterval = null;
    equityCurve = []; // Track equity over time
    closedTrades = []; // Track closed trades for metrics
    ...
    createSignalFromDecision(decision) {
        ...
        if (sizingAgent) {
            const sizedSignal = sizingAgent.calculateSignalSizing(baseSignal, this.marketDataBuffer, this.riskMetrics, this.positions.filter(p => p.status === 'open'));
            return sizedSignal;
        }
        return baseSignal;
    }
