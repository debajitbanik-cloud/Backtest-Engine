...
__exportStar(require("./core/BaseAgent"), exports);
__exportStar(require("./core/TradingEngine"), exports);
__exportStar(require("./config/defaultConfig"), exports);
__exportStar(require("./utils/helpers"), exports);
__exportStar(require("./data"), exports);
const TradingEngine_1 = require("./core/TradingEngine");
...
async function runDemo() {
    ...
    if (useRealData || fromFile)
        console.log(`Symbol: ${symbol} | Timeframe: ${timeframe}\n`);
    const engine = new TradingEngine_1.TradingEngine();
    const dataLoader = useRealData ? new data_1.BinanceDataLoader() : null;
    const fileLoader = fromFile ? new data_1.FileDataLoader('./data/shared') : null;
    engine.setCallbacks({
        onSignal: (signal) => {
            console.log(`\n📈 SIGNAL GENERATED:`);
    ...
    await engine.start();
    const processMarketDataArray = async (engine, data, stepLabel) => {
    ...
    if (useRealData && dataLoader) {
        ...
        try {
            ...
            const sharedFileLoader = new data_1.FileDataLoader('./data/shared');
        ...
        let basePrice = { BTCUSDT: 50000, ETHUSDT: 3000 };
        for (let i = 0; i < 100; i++) {
    ...
    const tradeMaster = engine.getManager().getSubAgent('trade_master_agent');
    if (tradeMaster) {
