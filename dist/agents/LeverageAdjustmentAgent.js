...
exports.LeverageAdjustmentAgent = void 0;
...
class LeverageAdjustmentAgent extends BaseAgent_1.BaseAgent {
    constructor(config) {
        const params = {
            ...config.parameters,
    ...
    calculateMarginRequirements(positions, riskMetrics) {
    ...
    calculateLeverageLimit(marginRatio, currentAvgLeverage) {
    ...
    handleHighMarginPeriod(marginRequirements, positions) {
        const openPositions = positions.filter(p => p.status === 'open');
        const avgLeverage = openPositions.length > 0
            ? openPositions.reduce((sum, p) => sum + p.leverage, 0) / openPositions.length
            : 1;
        ...
        if (marginRequirements.currentMarginRatio >= this.config.parameters.criticalMarginThreshold) {
            return this.createDecision('reduce', 0.95, `CRITICAL: Margin ratio at ${(marginRequirements.currentMarginRatio * 100).toFixed(1)}%. Reducing leverage from ${avgLeverage.toFixed(1)}x to ${newLeverageLimit}x (${reductionPercent.toFixed(1)}% reduction). Immediate position size reduction required.`, {
                newLeverageLimit,
        ...
        return this.createDecision('adjust', 0.85, `HIGH MARGIN: Margin ratio at ${(marginRequirements.currentMarginRatio * 100).toFixed(1)}%. Adjusting leverage limit from ${avgLeverage.toFixed(1)}x to ${newLeverageLimit}x (${reductionPercent.toFixed(1)}% reduction).`, {
            newLeverageLimit,
            currentMarginRatio: marginRequirements.currentMarginRatio,
    ...
    async onMarginUpdate(marginRequirements) {
        if (marginRequirements.isHighMarginPeriod) {
            return this.createDecision('adjust', 0.9, `Margin update: Ratio at ${(marginRequirements.currentMarginRatio * 100).toFixed(1)}%. Leverage limit set to ${marginRequirements.leverageLimit}x`, { newLeverageLimit: marginRequirements.leverageLimit });
        }
        return this.createDecision('hold', 0.5, 'Margin levels normalized');
    }
}
exports.LeverageAdjustmentAgent = LeverageAdjustmentAgent;
//# sourceMappingURL=LeverageAdjustmentAgent.js.map