...
exports.PositionSizingAgent = void 0;
...
class PositionSizingAgent extends BaseAgent_1.BaseAgent {
    constructor(config) {
        const params = {
            ...config.parameters,
    ...
    async analyze(marketData, positions, riskMetrics) {
        ...
        if (availableRisk <= 0) {
            return this.createDecision('hold', 0.9, `Portfolio risk at maximum (${(currentRisk * 100).toFixed(1)}%). No new positions allowed.`, { currentRisk, maxRisk: this.config.parameters.maxPortfolioRisk, availableRisk: 0 });
        }
        const sizingParams = this.calculateOptimalSizing(marketData, riskMetrics, availableRisk);
        return this.createDecision('adjust', 0.85, `Position sizing calculated: Risk per trade ${(sizingParams.riskPerTrade * 100).toFixed(2)}%, Max leverage ${sizingParams.maxLeverage}x, Position size $${sizingParams.positionSize.toFixed(2)}`, sizingParams);
    }
    calculateSignalSizing(signal, marketData, riskMetrics, openPositions = []) {
        const currentRisk = this.calculateCurrentRisk(openPositions, riskMetrics);
        const availableRisk = this.config.parameters.maxPortfolioRisk - currentRisk;
        if (availableRisk <= 0) {
        ...
        const leverage = Math.min(this.config.parameters.maxLeverage, Math.max(1, finalPositionValue / riskAmount));
    ...
    calculateCurrentRisk(positions, riskMetrics) {
        if (riskMetrics.marginAvailable + riskMetrics.marginUsed === 0)
            return 0;
        let totalRisk = 0;
    ...
    calculateOptimalSizing(marketData, riskMetrics, availableRisk) {
        ...
        const riskPerTrade = Math.min(this.config.parameters.riskPerTrade, availableRisk, kellySize);
        const maxLeverage = Math.min(this.config.parameters.maxLeverage, 1 / (riskPerTrade * 2));
    ...
    adjustForCorrelation(positions, newSignal) {
        const correlatedPositions = positions.filter(p => p.symbol === newSignal.symbol || this.areCorrelated(p.symbol, newSignal.symbol));
...
exports.PositionSizingAgent = PositionSizingAgent;
//# sourceMappingURL=PositionSizingAgent.js.map