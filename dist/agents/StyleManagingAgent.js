...
exports.StyleManagingAgent = void 0;
...
class StyleManagingAgent extends BaseAgent_1.BaseAgent {
    currentMode = 'swing';
    modeStartTime = Date.now();
    scalpingOpportunities = 0;
    constructor(config) {
        const params = {
            ...config.parameters,
    ...
    shouldSwitchToScalping(marginRatio, riskMetrics, marketData) {
        if (marginRatio > this.config.parameters.marginThresholdForScalping)
            return false;
        this.scalpingOpportunities = this.detectScalpOpportunities(marketData);
        if (this.scalpingOpportunities >= 2)
            return true;
    ...
    shouldSwitchToSwing(marginRatio, openPositions) {
        if (marginRatio > 0.7)
            return true;
        if (this.scalpingOpportunities < 1)
            return true;
        const scalpingPositions = openPositions.filter(p => p.mode === 'scalping');
    ...
    detectScalpOpportunities(marketData) {
    ...
    checkSwingExits(positions, marketData) {
    ...
    getModeStatusReasoning(marginRatio, scalpingCount, swingCount) {
    ...
    createScalpSignal(symbol, entryPrice, side) {
    ...
    createSwingSignal(symbol, entryPrice, side) {
...
exports.StyleManagingAgent = StyleManagingAgent;
//# sourceMappingURL=StyleManagingAgent.js.map